...
#include <sof/audio/impulse_noise/rtwtypes.h>
  ...
  * impnse_ceil(x) := -impnse_floor(-x), for all real x.
  *
 ...
 /*double floor16(double s_num, double s_den)
 {
     int16_T s_ans = 0;
...
*/
 ...
 //	if(jj0<20) {
 //	    if(jj0<0) { 	/* raise inexact if x != 0 */
 //		if(huge+x>0) {	/* return 0*sign(x) if |x|<1 */
