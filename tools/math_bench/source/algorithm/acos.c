...
/*
 * function th_rad_fxp = acos_fixpt(cdc_acos_th)
 *
 * th_rad_fxp  = fi(cordicacos(cdc_acos_th), 0, 32,29);
 *
 * Arguments	: const int32_t cdc_acos_th[21]
 *		  int32_t th_rad_fxp[21]
 * Return Type	: void
 */
void acos_fixpt(const int32_t cdc_acos_th[21], int32_t th_rad_fxp[21])
{
	...
	for (i = 0; i < 21; i++) {
		saturatedUnaryMinus = cdc_acos_th[i];
		if (saturatedUnaryMinus >= 0) {
			th_rad_fxp[i] = iScalarCordicACos(saturatedUnaryMinus, (int16_t)31, iv);
		} else {
			...
			th_rad_fxp[i] =
			    1686629713 - iScalarCordicACos(saturatedUnaryMinus, (int16_t)31, iv);
		}
	}
}
