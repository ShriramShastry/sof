...
/*
 * function [cdcCoSinTh] = cmpx_exp_fixpt(thRadFxp)
 *
 * Description
 * Compute cos(theta) + j*sin(theta).
 * theta contains the polar coordinates angle values, which are in the range [–pi, pi] radians.
 * The cordic cosine algorithm only converges when the angle is in the range [-pi/2, pi/2).
 * If an angle is outside of this range, then a multiple of pi/2 is added or subtracted from
 * the angle until it is within the range [-pi/2,pi/2).
 *
 * Arguments	: const int32_t thRadFxp[2047]
 *		  cint32_T cdcCoSinTh[2047]
 * Return Type	: void
 */
void cmpx_exp_fixpt(const int32_t thRadFxp[2047], cint32_T cdcCoSinTh[2047])
{
	...
	for (idx = 0; idx < 2047; idx++) {
		...
		for (b_idx = 0; b_idx < 31; b_idx++) {
			...
			xtmp = (xn >> ((uint32_t)((int32_t)(b_idx + 1))));
			ytmp = (b_yn >> ((uint32_t)((int32_t)(b_idx + 1))));
		}
		if (sign) {
			xn = -xn;
			b_yn = -b_yn;
		}
		cdcCoSinTh[idx].re = xn;
		cdcCoSinTh[idx].im = b_yn;
	}
}
